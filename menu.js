...
function buildScrollTopButton(){
  ...
  function syncState(){
    ...
    if(!atRest){
      btn.classList.remove('icon-hidden');
      restClickCount = 0;
      clearTimeout(restClickTimer);
    }
  ...
  btn.addEventListener('click', () => {
    ...
    if(btn.classList.contains('at-top')){
      if(isHomePage()){
        if(btn.classList.contains('icon-hidden')){
          btn.classList.remove('icon-hidden');
          restClickCount = 0;
        }else{
          wobble();
          restClickCount++;
          clearTimeout(restClickTimer);
          restClickTimer = setTimeout(() => { restClickCount = 0; }, 2000);
          if(restClickCount >= 3){
            btn.classList.add('icon-hidden');
            restClickCount = 0;
          }
        }
      }else{
