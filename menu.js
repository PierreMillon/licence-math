...
/* MENU_CHAPTERS = alias vers CHAPTERS (chapters.js, source unique,
   chargé avant ce fichier) — nom gardé pour ne pas devoir toucher
   tous les appels existants (buildDrawer, renderChapterNav plus bas,
   et mistakes.js/progression-page.js/revision.js). Contient TOUS les
   chapitres (actifs ou non) : filtrer avec .filter(ch => ch.active)
   au point d'usage quand il s'agit de navigation/affichage — voir
   buildDrawer/renderChapterNav — mais garder la liste complète pour
   les opérations qui doivent porter sur tout (ex. performSiteReset,
   plus bas, qui nettoie le state hebdo de chaque chapitre même
   inactif). */
const MENU_CHAPTERS = CHAPTERS;
...
/* Centralisé ici (refactoring du 10/08/2026 — était dupliqué dans
   progression.js et écrit en dur dans chaque fiches/*.js) car menu.js
   est le seul fichier chargé sur TOUTE page du site, y compris
   mistakes.html/notation.html/changelog.html qui n'ont pas weekly.js
   ni progression.js. Doit rester synchronisé avec le STATE_KEY passé
   à initFiche() par chaque fiches/*.js — logique seul a un suffixe
   _v2 (migration passée). */
...
/* ---------- rendu KaTeX partagé ---------- */
/* Centralisé ici (refactoring du 10/08/2026 — était dupliqué à
   l'identique sous 3 noms différents : typesetMath dans fiche-engine.js,
   typesetMistakes dans mistakes.js, typesetRevision dans revision.js).
   Appelé après avoir injecté du HTML contenant des délimiteurs KaTeX
   (\(...\) / \[...\]) pour les transformer en formules, puis enveloppe
   celles qui débordent (wrapOverflowingMath, juste en dessous). */
function typesetMath(el){
  ...
  /* Re-vérifie une fois les polices custom (Jersey 10 etc., font-display:
     optional) réellement chargées : si la première passe ci-dessus a eu
     lieu avec la police de secours (plus étroite) encore active, une
     formule mesurée "tient" à ce moment-là peut en réalité déborder une
     fois Jersey 10 en place (caractères plus larges) — sans reprise, ce
     débordement ne se corrige jamais puisque wrapOverflowingMath ne
     retourne jamais sur ce qu'il a déjà jugé correct. wrapOverflowingMath
     ignore déjà ce qui est enveloppé, donc ce second passage ne fait
     rien sur ce qui était déjà correctement traité. */
  if(window.wrapOverflowingMath && document.fonts && document.fonts.ready){
    document.fonts.ready.then(() => window.wrapOverflowingMath(el));
  }
}
window.typesetMath = typesetMath;

/* ---------- réglage « VALIDATION DES RÉPONSES » (notation.html) partagé ---------- */
/* Centralisé ici (refactoring du 10/08/2026 — était dupliqué à
   l'identique dans fiche-engine.js et revision.js). off = cocher une
   réponse la valide tout de suite. on (défaut pour tout nouveau
   visiteur depuis le 10/08/2026) = un bouton VALIDER apparaît,
   désactivé tant qu'aucune réponse n'est cochée (retour de Charles
   Boyer, qui cliquait parfois trop vite par réflexe) — les préférences
   déjà enregistrées sur un appareil restent inchangées. */
function confirmModeEnabled(){
  return window.getNotationPreference && window.getNotationPreference('confirmAnswer', 'on') === 'on';
}
window.confirmModeEnabled = confirmModeEnabled;
...
/* Une formule trop large pour l'écran (retour de Charles Boyer) ne doit
   ...
   par typesetMath (partagé, appelé depuis fiche-engine.js/mistakes.js/
   revision.js/progression-page.js). Les
...
/* Pourcentage de questions RÉPONDUES (pas forcément correctes — même
   ...
   est sans ouvrir chaque fiche. Lit le total directement dans CHAPTERS
   (chapters.js, chargé en premier sur TOUTE page) plutôt que dans
   CHAPTER_TOTALS (weekly.js, absent de mistakes.html/notation.html/
   changelog.html — bug trouvé le 19/08/2026 : le pourcentage restait
   vide dans le menu sur ces pages faute de source pour le total,
   alors que chapters.js le fournit déjà partout depuis la v144). */
function chapterCompletedPercent(chapterId){
  const chapter = CHAPTERS.find(c => c.id === chapterId);
  const total = (chapter && chapter.total) || 0;
...
/* Rythme du cœur du menu (11/08/2026, demande explicite) : plutôt que
   fixe, il varie selon le taux de bonnes réponses réel (correct/
   répondu, toutes fiches confondues) — pas la progression/complétion,
   juste la précision. 0% (ou pas encore assez de données) = rythme
   "pas sportif" un peu rapide ; 100% = rythme "sportif au repos" plus
   lent. Lu directement dans l1maths_progress (menu.js est chargé sur
   toutes les pages, contrairement à weekly.js/CHAPTER_TOTALS —
   accuracy ne dépend d'aucun total par chapitre, donc pas de
   dégradation à gérer ici). */
const HEART_BPM_FAST = 88;   // aucune bonne réponse
const HEART_BPM_SLOW = 50;   // 100% de bonnes réponses
const HEART_BPM_DEFAULT = 55; // pas encore de données (proche du rythme fixe d'origine, 1.1s)

function computeAccuracyPercent(){
  let correct = 0, completed = 0;
  try{
    const progress = JSON.parse(localStorage.getItem('l1maths_progress')) || {};
    Object.values(progress).forEach(p => {
      correct += (p && p.correct) || 0;
      completed += (p && p.completed) || 0;
    });
  }catch(e){ /* localStorage indisponible : on garde 0/0, fallback plus bas */ }
  if(completed <= 0) return null;
  return Math.max(0, Math.min(100, (correct / completed) * 100));
}

function applyHeartRate(){
  const hearts = document.querySelectorAll('.heart-icon');
  if(!hearts.length) return;
  const pct = computeAccuracyPercent();
  const bpm = pct === null ? HEART_BPM_DEFAULT : HEART_BPM_FAST + (HEART_BPM_SLOW - HEART_BPM_FAST) * (pct / 100);
  const duration = (60 / bpm).toFixed(3);
  hearts.forEach(h => { h.style.animationDuration = duration + 's'; });
}
...
function buildDrawer(){
  ...
  // Triés du moins avancé au plus avancé (le moins fait tout en haut,
  // le 100% tout en bas) — pour que l'ordre du menu pointe directement
  // vers ce qui reste à faire. Reste dans l'ordre fixe habituel si le
  // pourcentage n'est pas calculable sur cette page (weekly.js absent
  // de mistakes.html/notation.html/changelog.html) : trier des zéros
  // partout n'apporterait rien.
  // Chapitres pas encore donnés cette année (active:false, chapters.js)
  // : absents du tiroir de menu.
  const chaptersWithPct = MENU_CHAPTERS.filter(ch => ch.active).map(ch => {
    const chapterId = ch.file.replace('.html', '');
    return { ch, chapterId, pct: chapterCompletedPercent(chapterId) };
  });
  if(chaptersWithPct.every(c => c.pct !== null)){
    chaptersWithPct.sort((a, b) => a.pct - b.pct);
  }
  const itemsHTML = chaptersWithPct.map(({ ch, pct }) => {
...
function siteResetKeys(){
  // 'l1maths_logique_state' (sans _v2) : ancienne clé pré-migration,
  // gardée en plus de CHAPTER_STATE_KEYS.logique pour continuer à
  // nettoyer un très vieux visiteur jamais repassé par logique.html
  // depuis la migration.
  return Object.values(CHAPTER_STATE_KEYS).concat(['l1maths_logique_state']);
}
...
function performSiteReset(){
  ...
  /* Difficulté adaptative (11/08/2026) : une réinitialisation complète
     compte comme un crâne au même titre qu'une défaite hebdomadaire
     (choix explicite de Pierre) — baisse l'objectif de la semaine
     suivante de 10 points, comme une vraie défaite. */
  if(hadProgress && window.adjustWeeklyThreshold) window.adjustWeeklyThreshold(-0.10);
...
function undoSiteReset(){
  ...
  if(undo.skullIncremented && window.adjustWeeklyThreshold) window.adjustWeeklyThreshold(0.10);
  localStorage.removeItem(SITE_UNDO_KEY);
  window.location.reload();
}
...
const END_PHRASES = [
  ...
  // Ajoutées le 11/08/2026, deux aphorismes (au lieu d'une question
  // ouverte comme le reste de la liste — rupture de ton assumée,
  // demande explicite de Pierre).
  "CELUI QUI REPORTE SA RÉVISION D'UN JOUR LA REPORTERA DE SEPT",
  "ON PARDONNE TANT QUE L'ON AIME LE CHEVALIER ; LE DRAGON, LUI, NE PARDONNE RIEN",
  "JE NE DIRAIS PAS QUE J'AI PEUR DU DRAGON. DISONS JUSTE QUE J'AIMERAIS ÊTRE AILLEURS QUAND IL ARRIVE",
];
...
function renderChapterNav(){
  ...
  // Ne circule que parmi les chapitres actifs (chapters.js) — un
  // chapitre pas encore donné ne doit être atteignable par aucun
  // chemin, précédent/suivant inclus. Avec un seul chapitre actif
  // (ex. début d'année, seul PYTHON débloqué), il n'y a rien vers quoi
  // naviguer : pas de flèches plutôt que de pointer vers soi-même.
  const activeChapters = MENU_CHAPTERS.filter(ch => ch.active);
  const currentFile = window.location.pathname.split('/').pop();
  const idx = activeChapters.findIndex(ch => ch.file === currentFile);
  if(idx === -1 || activeChapters.length < 2){ el.innerHTML = ''; return; }

  const prev = activeChapters[(idx - 1 + activeChapters.length) % activeChapters.length];
  const next = activeChapters[(idx + 1) % activeChapters.length];
